...
try:
    # Bandit B314: defusedxml — XXE/billion-laughs/external-entity güvenliği için
    # xml.etree.ElementTree yerine kullanılır. API uyumlu.
...
class LogoTigerConnector(BaseConnector):
    ...
    def _parse_xml(self, data: bytes) -> ParseResult:
        ...
        if _ETREE_AVAILABLE:
            ...
            except Exception:  # ET.ParseError veya expat yokluğu → pure-Python fallback
                root = None
        if root is None:
            try:
...
def _node_to_str(node: Any) -> str:
    ...
    if _ETREE_AVAILABLE and ET is not None and hasattr(ET, "tostring"):
        try:
            return str(ET.tostring(node, encoding="unicode"))
