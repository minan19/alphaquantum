...
# Bandit B314: defusedxml — XXE/billion-laughs/external-entity güvenliği.
# Build için stdlib ET kullanıyoruz (güvenli), parse için defusedxml.
...
def generate_efatura_xml(invoice: EfaturaInvoice) -> bytes:
    ...
    # Register all namespace prefixes (default + named) BEFORE creating Element.
    # register_namespace global state ama tek doğru yer: serialization öncesi.
    # Manuel root.set("xmlns:...") çağrılarına gerek yok — duplicate attribute
    # üretir. ET kendiliğinden gerekli xmlns'leri ekler.
    ET.register_namespace("", NS[""])  # default namespace
