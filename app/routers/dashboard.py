"""A5.13 (part 4): Dashboard router — JSON signals + HTML legacy UI.

7 endpoint covering live dashboard signals, multi-company analytics, and the
legacy HTML admin dashboard (being replaced by Next.js frontend):

JSON endpoints (6):
- GET /api/v1/dashboard/live-signals          (S-311 live signals + S-335 ops)
- GET /api/v1/analytics/company-comparison    (S-313 holding-scope analytics)
- GET /api/v1/analysis                        (per-company analysis list)
- GET /api/v1/summary                         (DashboardSummary)
- GET /api/v1/insights                        (InsightItem list)
- GET /api/v1/dashboard-data                  (full DashboardDataResponse)

Legacy HTML (1):
- GET /dashboard                              (HTMLResponse, admin UI)

HTML dashboard is ~700 lines of CSS+HTML+JS in a Python f-string. It's the
pre-Next.js admin panel; kept for backward compatibility with bookmarked URLs.
`_render_dashboard` + 3 helpers (_render_company_card, _render_inventory_line,
_render_insight_item) ship together since they're tightly coupled.

`_build_dashboard_data` helper builds the shared snapshot used by summary,
insights, dashboard-data, and the HTML UI.
"""
...
def live_dashboard_signals(
    ...
    try:
        market_signal_count = len(
            request.app.state.market_repository.list_symbols()
        )
    except Exception:
...
# ── Multi-company comparison (S-313) ─────────────────────────────────────────
...
def _build_dashboard_data(request: Request) -> DashboardDataResponse:
    """Build the shared dashboard snapshot used by summary/insights/data/HTML."""
...
# ── Legacy HTML admin dashboard (~700 lines of CSS+HTML+JS in f-string) ─────
#
# Pre-Next.js admin UI. Kept for bookmark backward-compat; new frontend
# replaces this entirely. `_render_dashboard` is one massive f-string; do not
# refactor without care (escape rules + JS template literals collide).
...
def _render_dashboard(
    ...
    card_html = "".join(
        _render_company_card(company, analysis_map[company.name])
        for company in companies
    )
    ...
    async function loadMarketIntelligence(forceRefresh) {{
      ...
      const tags = (payload.pages || [])
        ...
        .map((item) => {{
          ...
          return `<span class="tag">${{escape(region)}}:${{escape(exchange)}}</span>`;
        }})
        .join("");
